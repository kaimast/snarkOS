...
for validator_index in $(seq 0 $((num_validators - 1))); do
  snarkos clean "--dev=$validator_index" "--network=$network_id" 2>/dev/null || true
  ...
  if (( validator_index == 0 )); then
    ...
    run_with_prefix "validator-$validator_index" $TASKSET1 snarkos start "--dev=$validator_index" --validator \
      "${common_flags[@]}" "--validators=$validators" --metrics --metrics-ip "$metrics_ip" \
      "--logfile=$log_file" "--rest=127.0.0.1:$rest_port"
  else
    # shellcheck disable=SC2086
    run_with_prefix "validator-$validator_index" $TASKSET2 snarkos start "--dev=$validator_index" --validator \
      "${common_flags[@]}" "--validators=$validators" --metrics --metrics-ip "$metrics_ip" \
      "--logfile=$log_file" "--rest=127.0.0.1:$rest_port"
  fi
...
if $skip_invalid_txs; then
  ...
  invalid_solution_json="$log_dir/invalid_solution.json"
  log "Generating invalid solution JSON..."
  if ! cargo run --example gen_invalid_solution --features bench -p snarkos-cli --quiet 2>"$log_dir/gen_invalid_solution.err" >"$invalid_solution_json"; then
    log "Failed to generate invalid solution. Build with: cargo build -p snarkos-cli --features bench --example gen_invalid_solution"
    cat "$log_dir/gen_invalid_solution.err" 1>&2
    exit 1
  fi
  if [ ! -s "$invalid_solution_json" ]; then
    log "Generated invalid solution file is empty"
    exit 1
  fi
  log "Running invalid solutions benchmark ($num_workers workers x $reqs_per_worker requests)..."
  python ./.ci/invalid_solutions_helper.py "$base_url" "$num_workers" "$reqs_per_worker" "$invalid_solution_json"
fi
