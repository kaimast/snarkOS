...
impl<N: Network> Consensus<N> {
    ...
    async fn process_unconfirmed_solutions(&self) -> Result<()> {
        ...
        for solution in solutions.into_iter() {
            ...
            match self.bft.primary().process_unconfirmed_solution(solution_id, Data::Object(solution)).await {
                Ok(true) => {}
                Ok(false) => debug!(
    ...
    async fn process_unconfirmed_transactions(&self) -> Result<()> {
        ...
        for transaction in transactions.into_iter() {
            ...
            match self.bft.primary().process_unconfirmed_transaction(transaction_id, Data::Object(transaction)).await {
                Ok(true) => {}
                Ok(false) => debug!(
...
impl<N: Network> BftCallback<N> for Consensus<N> {
    ...
    async fn process_bft_subdag(
        ...
        let result = self_
            .try_advance_to_next_block(subdag, transmissions_)
            .with_context(|| "Unable to advance to the next block");
...
impl<N: Network> Consensus<N> {
    ...
    async fn reinsert_transmission(
        ...
        match (transmission_id, transmission) {
            (TransmissionID::Ratification, Transmission::Ratification) => Ok(true),
