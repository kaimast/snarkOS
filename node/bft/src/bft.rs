...
pub struct BFT<N: Network> {
    ...
    /// Note: Always aquire a lock to `block_advancement_lock` before write-locking the DAG.
    ...
    /// Meta-lock to ensure there are no concurrent additions of certificates or commits.
    /// This is needed because the dag lock cannot be held across await points.
    ///
    /// TODO(kaimast): This lock will not be needed anymore once awaits introduced by channels are removed.
    block_advancement_lock: Arc<TMutex<()>>,
}
...
impl<N: Network> PrimaryCallback<N> for BFT<N> {
    ...
    async fn add_new_certificate(&self, certificate: BatchCertificate<N>) -> Result<()> {
        // Updating the DAG may commit a new leader certificate, so we need to hold the meta-lock.
        let _lock = self.block_advancement_lock.lock().await;
...
impl<N: Network> SyncCallback<N> for BFT<N> {
    ...
    async fn sync_dag_at_bootup(&self, certificates: Vec<BatchCertificate<N>>) -> Result<()> {
        // Hold the meta-lock while updating the DAG.
        let _lock = self.block_advancement_lock.lock().await;
    ...
    /// Insert a new certificate received through block sync.
    async fn add_new_certificates(&self, certificates: Vec<BatchCertificate<N>>) -> Result<()> {
        // Even though this code will not commit the leader certificate.
        // Still grab the meta-lock first, to avoid any in-process commits read inconsistent data.
        let _lock = self.block_advancement_lock.lock().await;

        // Update the DAG with the certificates.
        for certificate in certificates {
            self.update_dag(certificate).await?;
        }
...
impl<N: Network> BFT<N> {
    ...
    pub async fn shut_down(&self) {
        ...
        // Acquire the lock to ensure no block advancement is in progress.
        let _lock = self.block_advancement_lock.lock().await;
...
mod tests {
    ...
    async fn test_sync_bft_dag_at_bootup_shutdown() -> Result<()> {
        ...
        *bft.dag.write() = crate::helpers::dag::test_helpers::mock_dag_with_modified_last_committed_round(0);
        ...
        for certificate in post_shutdown_certificates.clone() {
            assert!(bootup_bft.update_dag(certificate).await.is_ok());
        }
        // Commit the second leader certificate.
    ...
    async fn test_commit_via_is_linked() {
        ...
        for certificate in certificates_by_round.into_iter().flat_map(|(_, certs)| certs) {
            storage.testing_only_insert_certificate_testing_only(certificate.clone());
            bft.update_dag(certificate).await.unwrap();
        }
